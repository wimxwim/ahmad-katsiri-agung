...
interface KursusDetailClientProps {
  slug: string;
  initialKursus: KursusItem | null;
  initialError: string | null;
}

export function KursusDetailClient({ slug, initialKursus, initialError }: KursusDetailClientProps) {
  const [kursus, setKursus] = useState<KursusItem | null>(initialKursus);
  ...
  const fetchKursus = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/v1/kursus?slug=${slug}`);
  ...
  async function handleEnroll() {
    ...
    if (!meRes || !meRes.ok) {
      window.location.href = `/masuk?portal=siswa&redirect=${encodeURIComponent(`/kursus/${slug}?auto_enroll=1`)}`;
      return;
    }
