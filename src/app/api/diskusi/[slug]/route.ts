...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    const rl = await checkRateLimit(`diskusi-detail:${ip}`, 20, 15000);
    ...
    if (rowIndex === -1) {
      return apiError("Diskusi tidak ditemukan", 404);
    }
    ...
    return apiError("Gagal memuat diskusi", 500);
  }
}
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const ip = ipFromRequest(req);
    const limit = await checkRateLimit(`balas:${ip}`, 8, 10_000);
    ...
    return apiSuccess();
  } catch (e) { console.error("POST /api/diskusi/[slug] error:", e);
