...
export async function POST(
  ...
  try {
    ...
    const rl = await checkRateLimit(`draft-reject-materi:${session.userId}`, 30, 60_000);
    if (!rl.allowed) return apiRateLimit(rl.retryAfter);

    const [updated] = await db
      .update(aiGeneration)
      .set({
        materiStatus: "rejected",
        updatedAt: new Date(),
      })
      .where(eq(aiGeneration.id, id))
      .returning();

    await appendEvent(`gen:${session.userId}`, "gen.materi_rejected", { generationId: id });

    return NextResponse.json({ success: true, data: updated });
  } catch (e) {
    console.error("Reject materi error:", e);
