# Ringkasan Klien — Ahmad Katsiri Agung
...
**AKAL Center** adalah ikhtiar sederhana yang sedang dibangun pelan-pelan — belum peluncuran, masih terus disempurnakan. Intinya, platform ini ingin meringankan beban tiga pihak sekaligus — murid, guru, dan kepala sekolah — lewat pendekatan diagnostik pembelajaran yang manusiawi.

- **Untuk murid:** setiap jawaban kuis dianalisis bukan cuma benar-salah, tapi juga waktu berpikir per soal. Sistem menghitung persentase penguasaan per topik (bukan cuma nilai rata-rata) dan memberi remedial yang pas — fokus 15 menit di topik yang belum dikuasai, bukan mengulang semua materi.
- **Untuk guru:** dashboard membantu menjawab "topik mana yang paling banyak murid belum paham?", "siapa yang mulai tertinggal?", "remedial apa yang cocok?" — semua muncul otomatis, guru tinggal klik dan murid dapat jalur belajar personal.
- **Untuk kepala sekolah:** ringkasan lampu hijau-kuning-merah per kelas, guru mana yang butuh pendampingan, tren pembelajaran sekolah — bukan grafik rumit, bukan alat menghakimi guru.

**Harga:** Rp20rb/bln/guru (dasar), Rp125rb/bln (tim 5 guru + AI), Rp200rb/bln (lengkap + dashboard kepala sekolah). AI Qwen 3.7 membantu bikin soal dan analisis kelas, tapi semua harus di-approve guru — guru tetap pemegang kendali penuh.
...
| Info | Detail |
|------|--------|
| **Nama klien** | Ahmad Katsiri Agung |
...
> 1. ~~Tidak perlu halaman login~~ → **UPDATE:** Setelah fitur evaluasi/kuis/siswa resmi ditambahkan, login gate diterapkan untuk melindungi data siswa. Landing page dan halaman konten publik tetap bisa diakses tanpa login.
...
- [x] Mockup desain dibuat (v1)
- [x] Mockup disetujui klien
...
- [x] Audit menyeluruh GPT 5.5 (backend, frontend, PRD, security, VPS) → lihat `audit-gpt55/`
- [ ] P0 Security hardening (tutup register guru publik, protect API, hilangkan mock, hardcoded secrets)
- [ ] P1 Fitur inti guru/siswa (DB persistence, dashboard guru real, kuis fix, import/export Excel)
