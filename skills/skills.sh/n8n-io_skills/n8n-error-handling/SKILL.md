...
Works on any node that calls a network service, not just HTTP Request. The engine retries on *any* error, with no per-status-code filter, and the engine caps `maxTries` at 5 and `waitBetweenTries` at 5000ms (`packages/core/src/execution-engine/workflow-execute.ts`). See `n8n-node-configuration` `HTTP_NODES.md` and `AI_NODES.md` for node-specific notes.
