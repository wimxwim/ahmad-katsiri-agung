...
**Counter-case:** `.all()` combined with another node's `.item` (a per-item lookup, e.g., `$('Get Tags').all().filter(tag => $('Search Posts').item.json.tag_ids.includes(tag.json.id))`) is real per-item work and should keep `executeOnce` off. See the `n8n-expressions` skill's executeOnce section for the full distinction.
...
- **Expressions aggregating across the full array via `$input.all()` / `$('Node').all()`.** Otherwise the aggregate runs per upstream item. (Per-item *lookups* using `.all()` filtered by another node's `.item` are the counter-case, see `n8n-expressions`.)
