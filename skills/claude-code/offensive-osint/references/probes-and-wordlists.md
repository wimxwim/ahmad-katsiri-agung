...
### 16.15.1 PaaS Origin Discovery — Vercel & Similar

Platforms like Vercel, Netlify, Railway serve apps behind their own edge/CDN. Even when a custom domain is proxied through Cloudflare/Akamai, the platform's native `*.vercel.app` / `*.netlify.app` URL **always resolves directly to origin IPs** — making it the single most reliable CDN bypass method.

**Vercel origin IP discovery:**

```bash
# Step 1: Find the Vercel project URL (from Vercel dashboard or via vercel CLI)
#   vercel ls | grep <project-name>

# Step 2: Resolve Vercel-native subdomain directly — bypasses any custom CDN
D="my-project"
dig +short "${D}.vercel.app" @1.1.1.1
dig +short "${D}.vercel.app" @8.8.8.8
dig +short "${D}.vercel.app" @9.9.9.9
```

**Known Vercel origin IP ranges** (observed across multiple resolvers/locations):

| Range | Source |
|-------|--------|
| `64.29.17.0/24` | Vercel edge (multiple deployments) |
| `216.198.79.0/24` | Vercel edge |
| `66.33.60.0/24` | `cname.vercel-dns.com` (infra domain) |
| `76.76.21.0/24` | `no-sni.vercel-infra.com` |

**SSL cert SNI leak:** Hit any Vercel origin IP directly with the correct `Host` header — the TLS certificate exposes `no-sni.vercel-infra.com` (internal infra domain) which further resolves to `cname.vercel-dns.com`:

```bash
# Hit origin IP directly — SNI cert leaks Vercel infra domain
curl -sv https://64.29.17.67/ -H "Host: my-project.vercel.app" 2>&1 | grep -i 'subject:\|CN=\|server certificate'
```

Expected: `subject: CN=no-sni.vercel-infra.com` — confirms this IP belongs to Vercel's infrastructure.

**Validate candidate origin:**

```bash
# Compare response from Vercel-native URL vs custom domain behind CF
diff <(curl -sk https://my-project.vercel.app/ | sha256sum) \
     <(curl -sk https://custom-domain.com/ | sha256sum)
# Same hash = confirmed origin
```

**Netlify origin discovery:**

```bash
D="project-name"
dig +short "${D}.netlify.app"
# Typical ranges: 75.2.0.0/16, 99.83.0.0/16, 13.32.0.0/14 (AWS CloudFront-based)
```

**Railway / Fly.io / Render origin discovery:**

```bash
# Railway: *.railway.app
dig +short "${D}.railway.app"

# Fly.io: *.fly.dev
dig +short "${D}.fly.dev"

# Render: *.onrender.com
dig +short "${D}.onrender.com"
```

**Why this works:** Platform-native subdomains (`*.vercel.app`) are not proxied by the customer's CDN — Cloudflare/Akamai only cover the custom domain's DNS zone, not Vercel's. The platform edge IPs are directly resolvable and often bypass all WAF/CDN rules.

**Severity (confirming this in a pentest):**
- Vercel-native URL resolves and serves identical content → **HIGH** `ORIGIN_EXPOSED_VIA_PLATFORM_DOMAIN` (bypasses WAF, rate limits, IP allowlisting on the custom CDN).
- Origin IPs not restricted to Cloudflare IP range → **CRITICAL** if direct IP access bypasses auth (rare for Vercel, common for self-hosted).

**Mitigation for defenders:** Vercel doesn't support IP allowlisting at the platform level. The only mitigation is to (a) use a custom domain exclusively and never share the `*.vercel.app` URL, (b) add authentication gating that checks the `Cf-Connecting-Ip` header from Cloudflare, or (c) put a separate WAF (e.g., Cloudflare Access) in front of the custom domain.
