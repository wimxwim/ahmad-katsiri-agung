...
railway env config --json                                # dump ALL env config (all services, full project scope — includes admin passwords, DB creds, API keys)
...
| Audit app for Railway migration readiness | [pre-deployment-audit.md](references/pre-deployment-audit.md) | Service dependency map, env var inventory, Docker build assessment, ephemeral storage audit, process management — run THIS FIRST before any migration |
| Ship code or manage releases | [deploy.md](references/deploy.md) | Deploy, redeploy, restart, build config, monorepo |
| Write or customize a Dockerfile for Railway | [dockerfile-patterns.md](references/dockerfile-patterns.md) | Multi-stage builds, C++ native deps, multiple processes in one container |
| Deploy AI agent (Hermes, bot) on Railway | [deploy-hermes.md](references/deploy-hermes.md) | Dockerfile, config, and env setup for running Hermes or other CLI agents as 24/7 gateway services |
...
| Access credentials for a deployed service | [access-credentials.md](references/access-credentials.md) | Extract admin passwords, API keys, DB creds from Railway env config to login to deployed dashboards |
...
### Deleted projects still appear in listings

`railway list` / `railway project list --json` shows **soft-deleted** projects. They have a `deletedAt` timestamp in JSON output. Trying to `railway link -p <deleted-project>` fails with "Project not found in workspace". When a user says "kok ada 2 padahal cuma 1", the extra one is likely a deleted project. Always check `--json` for `deletedAt`.

### Auto-select behavior

When a project has only one service, Railway CLI auto-selects it with: "No service specified — auto-selecting '<name>'" (non-error, expected).

### `railway env` non-interactive

`railway env` without a TTY returns "Environment must be specified when not running in a terminal". Use `railway env list` for environment names, `railway variable list --json` for a single service's variables, or `railway env config --json` for a full project dump (all services' variables, volume mounts, networking, source repos — the entire environment config in one shot).
